...
int main() {
    ...
    std::ifstream file("./Input_files/suite.txt");

    // Read the file.
    if (!file.is_open()) {
        std::cerr << "Error opening file 'suite.txt'." << std::endl;
        return 1;
    }
    ...
    std::cout << "The Server <U> is up and running using UDP on port <" << UDP_PORT << ">." << std::endl;
